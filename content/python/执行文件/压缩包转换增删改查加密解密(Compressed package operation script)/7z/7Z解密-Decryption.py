...
def process_archive(original_path: str, new_path: str, password: str, seven_zip: str) -> Tuple[bool, str]:
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # -o - 输出目录
            subprocess.run(
                [seven_zip, "x", original_path, f"-p{password}", f"-o{temp_dir}", "-y"],
                check=True,
        ...
        error_msg = e.stderr.decode(errors='ignore').strip() if e.stderr else str(e)
        # 清理可能创建的部分文件
...
def main(target_dir: str, password: str):
    ...
    # 初始化进度条
    with tqdm(archive_files, desc="解密进度", unit="file", colour="magenta") as pbar:
        success_count = 0
        for src_path in pbar:
            base_name = os.path.basename(src_path)
            new_filename = f"{os.path.splitext(base_name)[0]}_decrypted.7z"
            ...
            if is_success:
                success_count += 1
                pbar.write(f"[✓] 解密成功: {base_name}")
            else:
                pbar.write(f"[✕] 解密失败: {base_name} - {message}")

    # 输出统计信息
    print(f"\n处理完成！成功解密 {success_count}/{len(archive_files)} 个文件")
