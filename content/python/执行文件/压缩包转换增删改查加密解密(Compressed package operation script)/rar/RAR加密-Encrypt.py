...
OUTPUT_SUFFIX = "_encrypted"                                  # 新文件后缀
...
def encrypt_rar_file(
    ...
    try:
        # 解压原始文件到临时目录
        logger.info(f"解压文件: {os.path.basename(rar_path)}")
        subprocess.run([
            winrar_path, "x", "-ibck", "-y", rar_path, f"{temp_dir}\\"
        ], check=True, stdout=subprocess.DEVNULL)

        # 加密重新打包文件
        logger.info(f"创建加密文件: {os.path.basename(output_path)}")
        subprocess.run([
            winrar_path, "a", "-r", f"-p{password}", "-hp", "-y", "-ep1",
    ...
    try:
        ...
        for idx, rar_file in enumerate(files, 1):
            try:
                ...
                encrypt_rar_file(winrar, rar_file, OUTPUT_SUFFIX, ENCRYPTION_PASSWORD)
            except Exception as e:
